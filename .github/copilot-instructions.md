...
6. **Token 优化** - 规范加载应尽量精简，避免不必要的 token 消耗

---

## 🚀 v1.8.0 新特性：Token 优化

**问题**：`get_smart_standards` 返回完整规范内容，单次调用消耗 ~15000 tokens。

**解决方案**：新增 `get_compact_standards` 工具，支持三种模式：

| 模式 | Token 消耗 | 返回内容 | 适用场景 |
|------|-----------|---------|---------|
| `summary` | ~500 | 规范列表和描述 | 快速了解有哪些规范 |
| `key-rules` | ~2000 | 关键规则摘要 | **推荐**：日常开发 |
| `full` | ~15000 | 完整规范内容 | 需要详细参考时 |

**使用方式**：
```typescript
// 推荐：使用 key-rules 模式（默认）
get_compact_standards({ currentFile: "xxx.vue" })

// 仅获取规范列表
get_compact_standards({ mode: "summary" })

// 需要完整内容时
get_compact_standards({ mode: "full" })
```

**优化效果**：
- 日常开发 Token 消耗降低 **85%+**（15000 → 2000）
- 快速检查降低 **97%**（15000 → 500）
...
**核心原则**：定位根因，精准修复，避免反复修改

**禁止表面修复**：
| 表面现象 | ❌ 错误做法 | ✅ 正确做法 |
|----------|------------|------------|
| 边框缺失 | 直接加 border | 搜索全项目相关样式，查找冲突定义 |
| 元素错位 | 调整 margin | 检查父容器约束、flex/grid 布局 |
| 样式不生效 | 加 !important | 查看 DevTools 样式覆盖关系 |

**强制诊断流程**：
1. **搜索全项目** - 使用 grep_search 搜索相关选择器/函数
2. **确认无冲突** - 检查是否有多处定义相同/冲突的逻辑
3. **理解原意图** - 阅读原代码，理解为什么这样写
4. **从源头修复** - 修改根本原因，而非末端补救

**真实案例**（表格边框问题）：
- 表面现象：边框左粗右细
- 错误路径：加 border → 改 overflow → 加伪元素（反复3次）
- 根本原因：**两个 CSS 文件**定义了冲突的边框机制
- 正确修复：删除一处，保持单一来源

**详细规范**：`standards/workflows/problem-diagnosis.md`
